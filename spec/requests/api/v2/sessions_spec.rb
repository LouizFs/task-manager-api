...
RSpec.describe 'Sessions API', type: :request do
    ...
    describe 'Post /sessions' do
        ...
        context 'when then credentials are correct' do
            ...
            it 'returns the json data for the user with auth token' do
                ...
                expect(json_body[:data][:attributes][:'auth-token']).to eq(user.auth_token) #json_body é um helper
            end
            
        end

        context 'when the credentials are incorrect' do
            let(:credentials) {{email: user.email, password: "senhaerrada"}}
