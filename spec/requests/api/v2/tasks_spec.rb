...
RSpec.describe 'Tasks API' do
    ...
    describe 'GET /tasks' do
        context 'when no filter param is sent' do
            before do
                create_list(:task, 5, user_id: user.id) #cria uma lista de tarefas no factory
        ...
        context 'when filter and sorting params are sent' do
            let!(:notebook_task_1) {create(:task, title: 'Check if the notebookis broken', user_id: user.id)}
            ...
            let!(:other_task_2) { create(:task, title: 'buy e new car', user_id: user.id)}
            
            before do
                get '/tasks?q[title_cont]=note&q[s]=title+ASC', params: {}, headers: headers
            end

            it 'returns only the tasks matching andin the correctorder' do
    ...
    describe 'GET /tasks/:id' do
        let(:task) {create(:task, user_id: user.id)} #criando a tarefa
    ...
    describe 'POST /tasks' do
        ...
        context 'when the params are valid' do
            let(:task_params) {attributes_for(:task)} #hash com parametros validos para nossa tarefa
        ...
        context 'when the params are invalid' do
            ...
            it 'returns status code 422' do #status code 422 Unprocessable entity
    ...
    describe 'PUT /tasks/:id' do
        ...
        context 'when the params are invalid' do
            ...
            it 'returns json error fot title' do
                expect(json_body[:errors]).to have_key(:title)    
            end
